...
class TextEmbedder:
    def __init__(self, model_name: str = "all-MiniLM-L6-v2"):
        ...
        Loads the transformation model locally to map text into vector space.
        """
        print(f"⏳ Loading Embedding Model [{model_name}]...")
        self.model = SentenceTransformer(model_name)
        print(f"[SUCCESS] Model loaded and ready locally.")
    ...
    def generate_embeddings(self, processed_chunks: list) -> list:
        ...
        # Extracting text strings for bulk embedding generation (Fastest execution matrix)
        ...
        for idx, vector in enumerate(vectors):
            processed_chunks[idx]["embedding"] = vector.tolist()  # DB / Pickle compatibility
            
        print(f"[SUCCESS] Vectorization complete. Dimensionality: {len(vectors[0])} channels")
